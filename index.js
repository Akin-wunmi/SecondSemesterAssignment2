...
function setupEventListeners() {
  ...
  // Featured Product
  document.querySelector(".featured-add-to-cart").addEventListener("click", () => {
    addToCart("samurai-king", "Samurai King Resting", 100.0, "Premium");
  });
  ...
  // Desktop Filters
  document.querySelectorAll('.category-filters input[type="checkbox"]').forEach(input => {
  ...
  // Mobile Filters
  document.querySelector(".mobile-filter-toggle").addEventListener("click", openMobileFilters);
  document.getElementById("mobileFilterModal").querySelector(".close-modal-btn").addEventListener("click", closeMobileFilters);
  document.querySelector(".mobile-filter-actions .clear-btn").addEventListener("click", clearAllFilters);
  document.querySelector(".mobile-filter-actions .save-btn").addEventListener("click", saveMobileFilters);

  // Sync mobile filters to desktop
  document.querySelectorAll('#mobileFilterModal .filter-option input, #mobileFilterModal .price-option input').forEach(input => {
    input.addEventListener('change', (e) => {
      const syncId = e.target.id.replace('mobile-', '');
      const desktopEl = document.getElementById(syncId);
      if (desktopEl) {
